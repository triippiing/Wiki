...
- **`css/tokens.css`** is the warm stone / ochre palette, the four-font stack (Source Serif 4, Spectral, Public Sans, IBM Plex Mono), reset, base `body` styles, and shared `@keyframes`. Edit this to shift the design language across every runbook at once.
...
- **`css/sidebar.css`** is the 212px category rail (`.rail`) and its rail search. It collapses to an off-canvas drawer with a toggle below 1080px, and is hidden in print.
- **`js/chrome.js`** builds the code-block header bar (language label plus `⧉ COPY`) around every `.cmd` at load, binds the step accordions and their deep links, appends the Last reviewed cell, and injects the sidebar rail. Runbooks do **not** hand-write code-panel markup, a copy script, or any nav markup. Set `data-lang` on a `.cmd` to override the language label, which defaults to `shell`.
