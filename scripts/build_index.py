...
# content. Currently empty, since every category has at least one runbook.
...
def build_nav_data(sorted_cats: list[str], grouped: dict[str, list[dict]]) -> str:
    """Emit assets/js/nav-data.js: the page tree the sidebar rail is built from.

    chrome.js reads window.WIKI_NAV at load and injects the rail, so runbooks
    carry no nav markup of their own and a new page joins the sidebar the next
    time this script runs. Placeholder categories are excluded: the rail only
    links to pages that exist.
    """
    cats = [
        {
            "key":   c,
            "name":  CATEGORIES.get(c, {}).get("name", c.capitalize()),
            "pages": [
                {
                    "title": e["title"],
                    "href":  url_encode_path(e["rel_path"]),
                    "sub":   e["sub_tag"] or "",
                }
                for e in grouped[c]
            ],
    ...
    return (
        ...
        "window.WIKI_NAV = " + json.dumps(cats, indent=2, ensure_ascii=False) + ";\n"
    )
